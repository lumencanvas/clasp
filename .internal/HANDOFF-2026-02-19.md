...
1. **Needs deploy**: Sessions 5-7 changes are pushed to main but not yet deployed to production. Crates are published (3.4.0) so Docker build should now succeed. Next push to `deploy/relay/**` or `crates/**` will trigger the deploy pipeline.
2. **Remaining crates unpublished at 3.4.0**: Only `clasp-core`, `clasp-transport`, `clasp-router`, `clasp-discovery` were published. Other crates (`clasp-client`, `clasp-bridge`, `clasp-cli`, `clasp-wasm`, `clasp-embedded`) are still at 3.3.1 on crates.io. Publish when needed or as a batch.
...
5. **Private rooms not server-enforced**: Anyone who knows a room UUID can access it. Server-side access control would require room membership tracking in the relay.
...
8. **Namespace auth is client-side only**: Creator checks for namespace metadata updates are enforced in the composable, not the relay. A malicious client crafting raw `set()` calls could bypass them. Server-side namespace awareness would be needed for true enforcement.
9. **Namespace `__auth` path readable**: The `read:/chat/**` scope means any client can subscribe to `ns-meta/{ns}/__auth` and read the PBKDF2 hash. The hash uses 100k iterations which makes brute-force expensive, but for high-security namespaces, relay-side read restrictions on `__auth` paths would be ideal.
