...
1. **Needs deploy**: Sessions 5-6 changes are pushed to main but not yet deployed to production. Relay binary needs rebuild + push (new auth scopes for video + namespaces); frontend auto-deploys.
2. **Public channels timing**: Browse may not show rooms if SNAPSHOT is processed before subscriptions register. The params cache iteration fallback helps but may itself run before SNAPSHOT completes. A small delay or retry could improve reliability.
3. **Private rooms not server-enforced**: Anyone who knows a room UUID can access it. Server-side access control would require room membership tracking in the relay.
4. **Guest userId spoofing**: Guest endpoint accepts client-provided `user_id` — a malicious guest could claim another user's identity. Acceptable for current security model (guests are inherently anonymous), but real user verification would need server-side identity binding.
5. **Healthcheck noise**: Docker TCP healthcheck causes "Handshake not finished" errors in relay logs every 30s (harmless, cosmetic).
6. **Namespace auth is client-side only**: Creator checks for namespace metadata updates are enforced in the composable, not the relay. A malicious client crafting raw `set()` calls could bypass them. Server-side namespace awareness would be needed for true enforcement.
7. **Namespace `__auth` path readable**: The `read:/chat/**` scope means any client can subscribe to `ns-meta/{ns}/__auth` and read the PBKDF2 hash. The hash uses 100k iterations which makes brute-force expensive, but for high-security namespaces, relay-side read restrictions on `__auth` paths would be ideal.
...
cd deploy/relay && cargo build --release # clean
