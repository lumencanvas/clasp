...
class SignalFlowError(Exception):
    """SignalFlow client error"""
    pass


@dataclass
class SignalFlowBuilder:
    """Builder for SignalFlow client"""
    url: str
    name: str = "SignalFlow Python Client"
    features: List[str] = field(default_factory=lambda: ["param", "event", "stream"])
    token: Optional[str] = None
    reconnect: bool = True
    reconnect_interval: float = 5.0

    def with_name(self, name: str) -> "SignalFlowBuilder":
        """Set client name"""
        self.name = name
        return self

    def with_features(self, features: List[str]) -> "SignalFlowBuilder":
        """Set supported features"""
        self.features = features
        return self

    def with_token(self, token: str) -> "SignalFlowBuilder":
        """Set authentication token"""
        self.token = token
        return self

    def with_reconnect(self, enabled: bool, interval: float = 5.0) -> "SignalFlowBuilder":
        """Configure reconnection"""
        self.reconnect = enabled
        self.reconnect_interval = interval
        return self

    async def connect(self) -> "SignalFlow":
        """Build and connect"""
        client = SignalFlow(
            url=self.url,
            name=self.name,
...
class SignalFlow:
    ...
    @classmethod
    def builder(cls, url: str) -> SignalFlowBuilder:
        """Create a builder"""
        return SignalFlowBuilder(url=url)
    ...
    async def connect(self) -> None:
        """Connect to server"""
        if self._connected:
            raise SignalFlowError("Already connected")
        ...
        try:
            ...
            raise SignalFlowError(f"Connection failed: {e}") from e
    ...
    async def get(self, address: str, timeout: float = 5.0) -> Value:
        ...
        try:
            ...
            raise SignalFlowError("Get timeout")
    ...
    async def _send(self, msg: Dict[str, Any]) -> None:
        """Send message"""
        if not self._ws:
            raise SignalFlowError("Not connected")
    ...
    def _decode(self, data: bytes) -> Dict[str, Any]:
        ...
        if len(data) < 4 or data[0] != 0x53:
            raise SignalFlowError("Invalid frame")
    ...
    def _handle_message(self, msg: Dict[str, Any]) -> None:
        ...
        if msg_type == "SET":
            ...
            print(f"SignalFlow error: {msg.get('code')} - {msg.get('message')}")
    ...
    def _match_pattern(self, pattern: str, address: str) -> bool:
        ...
        # Convert SignalFlow pattern to fnmatch pattern
