...
use signalflow_core::{Message, PublishMessage, SetMessage, SignalType, Value};
use std::sync::Arc;
use tokio::sync::mpsc;
...
impl Bridge for MidiBridge {
    ...
    async fn start(&mut self) -> Result<mpsc::Receiver<BridgeEvent>> {
        ...
        let input_thread = std::thread::spawn(move || {
            ...
            let _conn = match midi_in.connect(
                ...
                move |_stamp, message, _| {
                    if let Some(msg) = midi_message_to_signalflow(message, &base_addr) {
        ...
        let output_thread = std::thread::spawn(move || {
            ...
            let mut conn = match midi_out.connect(&port, "signalflow-midi") {
                Ok(c) => c,
                Err(e) => {
    ...
    async fn send(&self, message: Message) -> Result<()> {
        ...
        if let Some(midi_msg) = signalflow_to_midi(&message, &self.midi_config) {
            sender
                .send(midi_msg)
...
fn midi_message_to_signalflow(message: &[u8], base_addr: &str) -> Option<Message> {
    if message.is_empty() {
        return None;
    }
...
fn signalflow_to_midi(message: &Message, _config: &MidiBridgeConfig) -> Option<Vec<u8>> {
    match message {
        Message::Set(set) => {
...
mod tests {
    ...
    #[test]
    fn test_midi_to_signalflow_note_on() {
        let msg = midi_message_to_signalflow(&[0x90, 60, 100], "/midi/test");
        assert!(msg.is_some());
    ...
    #[test]
    fn test_midi_to_signalflow_cc() {
        let msg = midi_message_to_signalflow(&[0xB0, 1, 64], "/midi/test");
        assert!(msg.is_some());
