...
use signalflow_transport::{Transport, TransportEvent, TransportReceiver, TransportSender, WebSocketTransport};
    ...
    match msg {
        Message::Set(set) => {
            ...
            for entry in subscriptions.iter() {
                ...
                if signalflow_core::address::glob_match(pattern, &set.address) {
        ...
        Message::Snapshot(snapshot) => {
            for param in &snapshot.params {
                ...
                for entry in subscriptions.iter() {
                    let (pattern, callback) = entry.value();
                    if signalflow_core::address::glob_match(pattern, &param.address) {
        ...
        Message::Publish(pub_msg) => {
            ...
            for entry in subscriptions.iter() {
                let (pattern, callback) = entry.value();
                if signalflow_core::address::glob_match(pattern, &pub_msg.address) {
