...
use signalflow_core::{codec, Message, HelloMessage, PROTOCOL_VERSION};
use signalflow_transport::UdpTransport;
use tokio::sync::mpsc;
...
pub async fn discover(port: u16, tx: mpsc::Sender<DiscoveryEvent>) -> Result<()> {
    ...
    loop {
        match timeout(discovery_timeout, receiver.recv_from()).await {
            Ok(Some((event, from))) => {
                if let signalflow_transport::TransportEvent::Data(data) = event {
                        ...
                        Ok((msg, _)) => {
                            if let Message::Welcome(welcome) = msg {
                                ...
                                let ws_url = format!(
                                    "ws://{}:{}/signalflow",
                                    from.ip(),
                                    signalflow_core::DEFAULT_WS_PORT
                                );
        ...
        while let Some((event, from)) = receiver.recv_from().await {
            if let signalflow_transport::TransportEvent::Data(data) = event {
                ...
                if let Ok((Message::Hello(_), _)) = codec::decode(&data) {
                    ...
                    let welcome = Message::Welcome(signalflow_core::WelcomeMessage {
                        version: PROTOCOL_VERSION,
