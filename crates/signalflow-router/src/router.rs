    ...
    match msg {
        ...
        Message::Get(get) => {
            if let Some(param_state) = state.get_state(&get.address) {
                let snapshot = Message::Snapshot(signalflow_core::SnapshotMessage {
                    params: vec![signalflow_core::ParamValue {
        ...
        Message::Query(query) => {
            ...
            let result = Message::Result(signalflow_core::ResultMessage { signals: vec![] });
