...
use signalflow_core::{Message, WelcomeMessage, PROTOCOL_VERSION};
use signalflow_transport::TransportSender;
...
impl Session {
    ...
    pub async fn send(&self, data: Bytes) -> Result<(), signalflow_transport::TransportError> {
    ...
    pub async fn send_message(&self, message: &Message) -> Result<(), signalflow_core::Error> {
        let data = signalflow_core::codec::encode(message)?;
        self.send(data)
            .await
            .map_err(|e| signalflow_core::Error::ConnectionError(e.to_string()))?;
        Ok(())
    }
