...
description: "Connect to a CLASP router using a known address when auto-discovery isn't available."
section: how-to
order: 1
---
# Manual Connection
...
// With TLS
const client = await new ClaspBuilder('wss://192.168.1.100:7330');
...
# Basic connection
client = await ClaspBuilder('ws://192.168.1.100:7330')

# With TLS
client = await ClaspBuilder('wss://192.168.1.100:7330')

# With options
client = await ClaspBuilder(
...
const client = await new ClaspBuilder('studio');
```
...
# In your code
const client = await new ClaspBuilder();  # Uses CLASP_ROUTER_URL
```
...
```javascript
const studio = await new ClaspBuilder('ws://192.168.1.100:7330');
const venue = await new ClaspBuilder('ws://192.168.1.101:7330');
