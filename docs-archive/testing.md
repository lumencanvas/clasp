---
title: "or specifically"
description: "cargo test --test oscechotest"
section: root
order: 3
---
## Testing Overview
...
- You don't need the richer orchestration/reporting from the clasp-e2e crate.

Over time, deeper protocol scenarios should live in clasp-e2e (see below), and this folder should mostly remain a small collection of critical smoke tests.

---

## 3. System / scenario / load tests (clasp-e2e)

- **Location**: `clasp-e2e/`
- **Crate**: `clasp-e2e`
- **Structure**:
  - Library + custom runner:
    - `clasp-e2e/src/lib.rs` — `TestSuite` and `TestResult` types.
    - `clasp-e2e/src/tests/*.rs` — grouped tests (OSC, MIDI, Art-Net, CLASP-to-CLASP, security, load, helpers).
    - `clasp-e2e/src/main.rs` — `run-all-tests` orchestrator.
  - Binaries (`clasp-e2e/src/bin/*.rs`) — focused suites and scenarios:
  ...
  - Benchmarks:
    - `clasp-e2e/benches/throughput.rs`
...
```bash
# Bridges
cargo run -p clasp-e2e --bin http_integration_tests
cargo run -p clasp-e2e --bin mqtt_integration_tests
cargo run -p clasp-e2e --bin websocket_bridge_tests

# Transports
cargo run -p clasp-e2e --bin transport_tests
cargo run -p clasp-e2e --bin udp_tests
cargo run -p clasp-e2e --bin quic_tests
cargo run -p clasp-e2e --bin p2p-connection-tests --features p2p

# System behavior
cargo run -p clasp-e2e --bin bundle_tests
cargo run -p clasp-e2e --bin lock_tests
cargo run -p clasp-e2e --bin e2e-protocol-tests

# Security / load
cargo run -p clasp-e2e --bin security-tests
cargo run -p clasp-e2e --bin security-pentest
cargo run -p clasp-e2e --bin load-tests
```
...
- **Add to `tests/integration/` if:**
  ...
  - You don't need the full clasp-e2e harness.
...
3. **`clasp-e2e/`** for everything heavier or more scenario-driven.
