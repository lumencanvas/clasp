---
title: "Embedded Sensor Node Tutorial"
description: "Build a wireless sensor node using CLASP on a microcontroller."
section: tutorials
order: 5
---
...
void handleMessage(char* payload) {
  ...
  // Use ArduinoJson for reliable parsing (add to lib_deps)
  StaticJsonDocument<256> doc;
  DeserializationError error = deserializeJson(doc, payload);
  if (error) return;

  const char* address = doc["address"];
  if (address && strstr(address, "/led/brightness")) {
    float brightness = doc["value"].as<float>();
...
fn main() -> ! {
    ...
    let mut wifi = init_wifi(peripherals.WIFI, "SSID", "PASSWORD");
    let mut sensor = init_dht22(peripherals.GPIO4);

    let mut clasp = Client::new();

    loop {
        // Read sensor (implementation depends on your HAL)
        let temperature = sensor.read_temperature();

        // Prepare a CLASP frame (just bytes, no allocator needed)
        let frame = clasp.prepare_set(
            "/sensors/esp32/temperature",
            Value::Float(temperature as f64)
        );

        // Send the raw frame via your transport (UDP, TCP, etc.)
        wifi.send_udp("192.168.1.100:7330", &frame);
...
> [!NOTE]
> The hardware initialization (`init_wifi`, `init_dht22`) depends on your specific board and HAL crate. See [esp-hal](https://github.com/esp-rs/esp-hal) for ESP32 examples.

`clasp-embedded` is transport-agnostic and uses only ~3.6KB RAM. It generates raw protocol frames that you send over whatever transport your hardware supports.
