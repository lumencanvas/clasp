/**
 * SignalFlow Protocol v2 - Minimal Reference Implementation
 * 
 * This demonstrates that SignalFlow can be implemented in ~300 lines.
...
class SignalFlow {
  ...
  connect() {
    return new Promise((resolve, reject) => {
      ...
      this.ws = new WebSocket(this.url, 'signalflow.v2');
  ...
  _handleMessage(msg, qos, timestamp) {
    switch (msg.type) {
      ...
      case MSG.ERROR:
        console.error('SignalFlow error:', msg.code, msg.message);
...
// Export
module.exports = { SignalFlow, encodeFrame, decodeFrame, MSG, QOS };

// Usage example (when run directly)
if (require.main === module) {
  console.log(`
SignalFlow v2 Minimal Implementation

Usage:
  const { SignalFlow } = require('./signalflow-minimal');
  
  const sf = new SignalFlow('wss://localhost:7330');
  await sf.connect();
  
  // Subscribe
  sf.on('/lumen/scene/*/layer/*/opacity', (value, address) => {
  ...
  sf.bundle([
    { set: ['/light/1', 1.0] },
    { set: ['/light/2', 0.0] }
  ], { at: sf.time() + 100000 });  // 100ms from now
`);
}
